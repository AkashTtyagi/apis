...
- **Cost of Living Index** — Dropdown (Low/Medium/High/Very High)
  - Used for manual limit adjustments
...
- **Icon/Image** — Upload (optional)
  - Visual representation for dashboard
...
- **Master Data Source** — Sync from HRMS (Departments, Locations, Employees)
...
**Note:** Project/Cost Center allocation will be added when Timesheet module is implemented
...
- **Distance Calculation Method** — Dropdown:
  ...
  - System Predefined Routes (manual entry)
...
- **Add Exchange Currency** — Button
  ...
  - **Exchange Rate** (mandatory) — Numeric input (manual entry)
...
- **Default Route Provider** — Dropdown:
  - Manual Entry (employee specifies distance)
  - Predefined Routes (admin-configured routes)
...
- **Enable Validation Rules** — Toggle (Yes/No)

  - **Violation Types Detected**:
    - Amount exceeds limit
    - Missing receipt
    - Duplicate expense (same date, amount, category)
    - Date discrepancies
    - Category mismatch
...
- **Upload Bank Confirmation** — File upload (optional)
  ...
  - Manual matching and mark as paid
...
- **Manual Matching** — For transactions
...
Manual validation and comprehensive audit capabilities.

### Manual Policy Violation Detection

**Validation Rules**
- **Rule-Based Detection** — Configure validation checks

  - **Violation Categories**:
    - Amount exceeds configured limit
    - Missing mandatory receipt
    - Duplicate expense detected (same date, amount, category, employee)
...
- **Matching Logic** — System checks for duplicates:
  - Same amount ± 5%
...
- **Action**:
  ...
  - Allow confirmation if intentional
...
The following features have been removed as they cannot be implemented with the current HRMS system architecture (Node.js/Express/MySQL):

**1. AI/ML Features**
- AI Limit Advisor with machine learning recommendations
...
- Google Maps integration for automatic route calculation
- OpenStreetMap integration
- Third-party OCR providers (AWS Textract, Google Cloud Vision, Azure)
  - **Note:** OCR will be built as custom in-house solution when needed
- Real-time currency exchange rate APIs

**3. Advanced Cloud Services**
- Cloud-based OCR with automatic data extraction (will use custom in-house OCR instead)
- Real-time GPS location capture
- Automatic bank transaction matching with AI

**4. Mobile-Specific Features**
- Offline mode with sync capability
- Automatic camera launch for receipts
- GPS auto-capture for mileage tracking

### Modules Deferred to Future Implementation

**1. Vendor & Merchant Management**
- Will be implemented later in detail
- Basic vendor tracking may be included in initial version

**2. Project & Cost Center Mapping**
- Will be implemented along with Timesheet module
- Deferred to maintain consistency with time tracking and project billing

### What Remains (Implementable Features)

All other features in this document CAN be implemented with the current stack:
- Manual expense filing and approval workflows (via HRMS workflow engine)
- Rule-based policy validation
- Bank sheet generation for payments
- Manual reconciliation processes
- Comprehensive reporting and dashboards
- Audit trail and compliance tracking
- Manual duplicate detection
- Currency management with manual rate entry
- Mileage tracking with manual distance entry or predefined routes
- Advance payment management
- Custom in-house OCR (when implemented)

---

**Document Version:** 2.1 (Streamlined for Current Implementation)
**Last Updated:** 2025-11-13
**Technology Stack:** Node.js, Express, MySQL/Sequelize, HRMS Integration
**Deferred Modules:** Vendor Management, Project & Cost Center Mapping (will be done with Timesheet)
**OCR:** Custom in-house solution (no external providers)
